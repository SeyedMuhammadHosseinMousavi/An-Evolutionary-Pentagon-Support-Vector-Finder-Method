...
% Loading Data
load UserKnowladgeModeling.mat;
User=UserKnowledge;
UserDat=User(:,1:end-1);
UserLbl=User(:,end);
X = UserDat;
OriginData=X;
...
OutlierRemFirst = rmoutliers([ABCClusRes label],'movmedian',4);
...
class3(1:sizedata(1,2))=0;
class4(1:sizedata(1,2))=0;
for i=1:sizedata(1,1)
    ...
    elseif NewLabel(i)== 3
        class3(i,1:end)= OutlierRemFirst(i,:);
    elseif NewLabel(i)== 4
        class4(i,1:end)= OutlierRemFirst(i,:);
    end
end
% Removing zero rows
indc1 = find(sum(class1,2)==0) ;
indc2 = find(sum(class2,2)==0) ;
indc3 = find(sum(class3,2)==0) ;
indc4 = find(sum(class4,2)==0) ;
class1(indc1,:) = [] ;
class2(indc2,:) = [] ;
class3(indc3,:) = [] ;
class4(indc4,:) = [] ;
...
j=1;
for i=1:psvdiv
psvdat{i}=OutlierRemFirst(j:j+5,1:end-1);
j=j+5;
end;
% area
for i=1:psvdiv
...
j=1;
for i=1:psvdiv
forfin{i}=OutlierRemFirst(j:j+5,:);
j=j+5;
end;
%
threshold=0.1;
for i=1:psvdiv
    if penarea(i)<threshold
